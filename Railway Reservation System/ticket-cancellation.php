...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO ticket_cancellations (passenger, trainNumber, train, departure, arrival, fare, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $passenger, $trainNumber, $train, $departure, $arrival, $fare, $status);
    $stmt->execute();
    $stmt->close();
}

// Retrieve ticket cancellations
$ticketCancellations = [];
$result = $conn->query("SELECT * FROM ticket_cancellations");
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $ticketCancellations[] = $row;
    }
}
$conn->close();
?>
...
<head>
    ...
    <title>Ticket Cancellation - BiyaheRail</title>
        ...
        <div class="row">
            ...
            <main class="col-md-10 ms-sm-auto col-lg-10 px-md-4">
                <div class="pt-3 pb-2 mb-3">
                    ...
                    <p>Manage your ticket cancellations here.</p>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php foreach ($ticketCancellations as $cancellation): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($cancellation['passenger']); ?></td>
                                    <td><?php echo htmlspecialchars($cancellation['trainNumber']); ?></td>
                                    <td><?php echo htmlspecialchars($cancellation['train']); ?></td>
                                    <td><?php echo htmlspecialchars($cancellation['departure']); ?></td>
                                    <td><?php echo htmlspecialchars($cancellation['arrival']); ?></td>
                                    <td><?php echo htmlspecialchars($cancellation['fare']); ?></td>
                                    <td><?php echo htmlspecialchars($cancellation['status']); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                ...
                <!-- Add Ticket Cancellation Form -->
                <div class="mt-4">
                    <h4>Add New Ticket Cancellation</h4>
                    <form method="POST" action="">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="status" name="status" value="Cancelled" readonly>
                        </div>
                        <button type="submit" class="btn btn-primary">Add Cancellation</button>
                    </form>
                </div>
            </main>
        </div>
